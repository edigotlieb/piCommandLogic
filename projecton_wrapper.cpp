...
read_results serial_readMSG(int serial_fd)
{
	...
	mavlink_message_t message[MAX_NUM_OF_MESSAGES];
	...
	while (num_of_messages < MAX_NUM_OF_MESSAGES)
	{
		...
		if (read(fd, &cp, 1) > 0)
		{
			...
			msgReceived = mavlink_parse_char(MAVLINK_COMM_1, cp, &message[num_of_messages], &status);
			if (msgReceived)
			{
				num_of_messages++;
			}
	...
	if (msgReceived)
	{
		for (int i = 0; i < MAX_NUM_OF_MESSAGES; i++)
		{
			...
			switch (message[i].msgid) {
				case MAVLINK_MSG_ID_ATTITUDE:
				{
					mavlink_attitude_t attitude;
					mavlink_msg_attitude_decode(&message[i], &attitude);
					// check trigger flag
					// send trigger
					// capture
					// process
				...
				case MAVLINK_MSG_ID_NAMED_VALUE_INT:
				{
					...
					mavlink_msg_named_value_int_decode(&message[i], &named_int);
